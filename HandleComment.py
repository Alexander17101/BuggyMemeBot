...
def SelectReplyMsg(comment):
    ...
    try:
        ...
        for x in range(0, len(NoobHelpKeywordList)): #bug facts
            if NoobHelpKeywordList[x] in comment.body:
                msg = Reply.NoobHelp()
                KeywordInComment = True
...
def ReplyToComment(msg, comment):
    try:
        if comment.author != "BuggyMemeBot" and comment.id not in alreadyDone:
            comment.reply(msg)
            print("Comment delivered! I wrote: " + msg)
            alreadyDone.append(comment.id)
    except Exception as e:
        print(e)
    finally:
        if len(alreadyDone) > 500:
            alreadyDone.pop(0);     #goddamn it that so fucking dirty code, but it still works. Lets just assume that this sub will never have 500 comments in 3 posts
